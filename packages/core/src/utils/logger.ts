 ...
 *
 * Mode is set via CTXLOOM_LOG_MODE env var (cli | json). The CLI entry
 * point sets this to "cli" before importing any module that uses the
 * logger, so structured-JSON noise doesn't leak into ctxloom index /
 * status / etc.
 *
...
function getMode(): LogMode {
  ...
  // Auto-detect: if the process was invoked with CLI arguments, we're a
  // human-facing command; otherwise (bare `ctxloom`) we're the MCP server.
  // Checked at write time so ESM-hoisted imports can't race the detection.
...
function writeCli(level: LogLevel, msg: string, extra?: Record<string, unknown>): void {
  // CLI mode: suppress info/debug entirely (they're noise during interactive
  // commands). Warn/error pretty-print as a single colored line.
  ...
  if (extra && Object.keys(extra).length > 0) {
    // Compact extras as `key=value` pairs, dimmed
