...
export class GitHistoryMiner {
    ...
    async *stream(opts) {
        if (opts?.sinceSha !== undefined && !/^[0-9a-f]{40}$/i.test(opts.sinceSha)) {
            throw new Error(`GitHistoryMiner: invalid sinceSha — expected 40-char hex SHA, got: ${opts.sinceSha}`);
        }
    ...
    async *fetchRawLogStream(logArgs) {
        ...
        await new Promise((resolve, reject) => {
            child.on('close', (code) => {
                if (code === 0 || code === null) {
                    ...
                    const knownEmpty = /does not have any commits|bad default revision|unknown revision|your current branch/i.test(stderr);
                    if (knownEmpty) {
    ...
    parseRecord(record, bulkThreshold, excludePaths) {
    ...
    buildEvent(headerLine, numstatBlock, bulkThreshold, excludePaths) {
    ...
    parseNumstat(block, excludePaths) {
        const files = [];
        for (const line of block.split('\n')) {
            ...
            if (excludePaths.some((prefix) => filePath.startsWith(prefix)))
                continue;
...
//# sourceMappingURL=GitHistoryMiner.js.map