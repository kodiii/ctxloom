...
export interface CoChangeStats {
  nodeA: string; // always lexicographically ≤ nodeB
  nodeB: string;
  sharedCommits: number;
  countA: number;
  countB: number;
  lastSharedTimestamp: number; // unix seconds
  jaccard: number;
}
...
export interface CoChangeQuery {
  ...
  now?: number;           // unix seconds, for testability; defaults to Date.now()/1000
...
export class CoChangeIndex {
  private readonly pairMap: Map<string, CoChangeStats> = new Map();
  private readonly nodeCounts: Map<string, number> = new Map();
  ...
  private updatePair(fileA: string, fileB: string, timestamp: number): void {
  ...
  private pairsForNode(node: string): CoChangeStats[] {
    const result: CoChangeStats[] = [];
