            ...
            switch (node.type) {
                ...
                case 'export_statement': {
                    ...
                    if (hasDefault) {
                        ...
                        const innerChild = node.children.find(c => c?.type === 'function_declaration'
                ...
                case 'class_declaration': {
                    ...
                    if (nameNode) {
                        ...
                        const methods = methodNodes
                            ...
                            .filter(Boolean);
                        const methodRanges = methodNodes
                            .map(c => {
                ...
                case 'class_definition': {
                    ...
                    if (nameNode) {
                        ...
                        const methods = (body?.children ?? [])
                            .filter((c) => c !== null && c.type === 'function_definition')
                            .map(c => c.childForFieldName?.('name')?.text ?? '')
                            .filter(Boolean);
                        nodes.push({
                            type: 'class',
        ...
        const walk = (node) => {
            switch (node.type) {
                ...
                case 'mod_item': {
                    ...
                    const body = node.childForFieldName?.('body');
                    if (body)
                        return; // `mod foo { ... }` inline block — skip
                    const nameNode = node.childForFieldName?.('name');
                    if (nameNode) {
                        nodes.push({
                            type: 'import',
            ...
            switch (node.type) {
                case 'class_declaration': {
                    ...
                    if (nameNode) {
                        ...
                        const body = node.childForFieldName?.('body');
                        const methods = (body?.children ?? [])
                            .filter((c) => c !== null && (c.type === 'method_declaration' || c.type === 'constructor_declaration'))
                            .map(c => c.childForFieldName?.('name')?.text ?? '')
                            .filter(Boolean);
                        const methodRanges = (body?.children ?? [])
                            .filter((c) => c !== null && (c.type === 'method_declaration' || c.type === 'constructor_declaration'))
                            .map(c => {
                            const mName = c.childForFieldName?.('name')?.text ?? '';
                            return mName ? { name: mName, signatureLine: c.startPosition.row + 1 } : null;
            ...
            switch (node.type) {
                ...
                case 'struct_declaration': {
                    ...
                    if (nameNode) {
                        const body = node.childForFieldName?.('body');
                        const methods = (body?.children ?? [])
                            .filter((c) => c !== null && c.type === 'method_declaration')
            ...
            switch (node.type) {
                ...
                case 'class': {
                    ...
                    if (nameNode) {
                        const body = node.childForFieldName?.('body');
                        const methods = (body?.children ?? [])
                            .filter((c) => c !== null && (c.type === 'method' || c.type === 'singleton_method'))
                            .map(c => (c.childForFieldName?.('name') ?? c.children.find(ch => ch?.type === 'identifier'))?.text ?? '')
                            .filter(Boolean);
                        nodes.push({
                            type: 'class',
